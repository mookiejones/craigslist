...
const debug = debug_1.default('craigslist'), DEFAULT_HTTP_PORT = 80, DEFAULT_HTTPS_PORT = 443, DEFAULT_MAX_REDIRECT_COUNT = 5, DEFAULT_RETRY_COUNT = 3, DEFAULT_TIMEOUT = 30000, EVENT_REDIRECT = 'redirect', EVENT_REQUEST = 'request', EVENT_RESPONSE = 'response', FIRST_TRY = 1, HTTP_ERROR_CODE_THRESHHOLD = 400, HTTP_ERROR_CODE_RETRY_THRESHHOLD = 500, 
// reference: https://en.wikipedia.org/wiki/List_of_HTTP_status_codes#3xx_Redirection
HTTP_PROXY_REQUIRED = 305, HTTP_REDIRECT_CODE_PERM = 301, HTTP_REDIRECT_CODE_TEMP = 302, HTTP_REDIRECT_NEW_CODE_PERM = 308, HTTP_REDIRECT_NEW_CODE_TEMP = 307, HTTPS_RE = /^https\:?/i, REQUEST_OPTIONS = [
    'agent',
    'auth',
    'family',
    'headers',
    ...
    'socketPath',
    'timeout'
], SECURE_PROTOCOL_RE = /^https/i, TOP_LEVEL_REDIRECT_RE = /^\/{2}([a-z0-9\-\_\.]*)([\/a-z\-\_\.]*)/i;
function _augmentRequestOptions(options) {
    ...
    REQUEST_OPTIONS.forEach((field) => {
        let value = core_js_1.default.Validation.coalesce(options[field], self.settings[field]);
        if (!core_js_1.default.Validation.isEmpty(value)) {
    ...
    augmented.maxRetries = core_js_1.default.Validation.coalesce(augmented.maxRetries, DEFAULT_RETRY_COUNT);
    // ensure rawStream setting is applied if not supplied
    augmented.rawStream = core_js_1.default.Validation.isEmpty(augmented.rawStream) ?
        false :
        augmented.rawStream;
    // ensure default timeout is applied if one is not supplied
    augmented.timeout = core_js_1.default.Validation.coalesce(augmented.timeout, DEFAULT_TIMEOUT);
    // create `path` from pathname and query.
    augmented.path = core_js_1.default.Validation.coalesce(augmented.path, augmented.pathname);
    return augmented;
}
function _exec(options, data, tryCount, callback) {
    if (typeof data === 'function' && core_js_1.default.Validation.isEmpty(callback)) {
        callback = data;
        /*eslint no-undefined:0*/
        data = undefined;
        tryCount = FIRST_TRY;
    }
    if (typeof tryCount === 'function' && core_js_1.default.Validation.isEmpty(callback)) {
        callback = tryCount;
        tryCount = FIRST_TRY;
    }
    data = data || '';
    ...
    exec = new Promise(function (resolve, reject) {
        ...
        if (!core_js_1.default.Validation.isEmpty(options.proxy)) {
            let host = options.host || options.hostname, proxy = url_1.default.parse(options.proxy);
            ...
            if (options.path.indexOf(host) < 0) {
                ...
                options.path = [
                    ...
                    path_1.default.join(host, options.path)
                ].join('://');
            }
        ...
        let makeRequest = function () {
            ...
            let req = (options.secure ? https_1.default : http_1.default).request(options, (res) => {
                let chunks = [], context = {
                    headers: res.headers,
                ...
                if (redirect) {
                    if (core_js_1.default.Validation.isEmpty(context.headers.location)) {
                    ...
                    let redirectUrl = url_1.default.parse(context.headers.location);
                ...
                res.once('end', () => {
                    let body = chunks.join(''), retry = context.statusCode >= HTTP_ERROR_CODE_RETRY_THRESHHOLD &&
    ...
    return core_js_1.default.Validation.promiseOrCallback(exec, callback);
}
class Request extends events_1.default.EventEmitter {
    ...
    get(options, callback) {
        ...
        options = this;
        _augmentRequestOptions(options);
        options.method = 'GET';
        return this;
